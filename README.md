...
The `checkout_url` returned by `/checkout/initialize` is generated by the Ghion backend. The backend builds it from the `CHECKOUT_BASE_URL` environment variable (falling back to `GATEWAY_BASE_URL`).

If your checkout links point to the wrong domain (e.g. `https://api.ghion.financial/checkout/...` instead of `https://app.ghion.financial/checkout/...`), set the backend's `CHECKOUT_BASE_URL` to the frontend domain that serves the checkout pages (`https://app.ghion.financial` in production).
